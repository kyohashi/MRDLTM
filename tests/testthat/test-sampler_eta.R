test_that("sample_eta runs without error and updates values", {
  ...
  active_data = data.frame(
    ...
    time = sample(1:n_time, 10, replace = TRUE),
    y_cit = rbinom(10, 1, 0.5)
  )

  state = init_state(active_data, n_item, n_cust, n_topic, n_time, 1, p_dim)
  ...
  # --- Execution ---
  expect_error(
    sample_eta(active_data, state, Dc, n_cust, n_topic, n_time, p_dim),
    NA
  )
  ...
  expect_equal(dim(state$eta_zct), c(n_topic - 1, n_cust, n_time))
  expect_equal(dim(state$omega_zct), c(n_topic - 1, n_cust, n_time))
})
